...
class InputAutocomplete extends Input {
  ...
  constructor(inputId, props, opts) {
    super(inputId, props);
    this._value = props.value ? props.value : null;
    this.el.classList.add('autocomplete');
    this.options = list('ul', __InputAutocompleteItem);
    this.update(opts);
    ...
    this.input.onkeyup = evt => {
      ...
      if (this.input.value.length > 2) {
        //opts.map()
        mount(this.el, this.options);
      } else {
        unmount(this.el, this.options);
      }
      this.modified = true;
      // const event = new CustomEvent("inputchange", { detail: { el: this.el, obj: this, data: this.data } }); // old:  {detail: this.input.value}
      // document.dispatchEvent(event);
  ...
  constructor() {
    ...
    this.el.onclick = evt => {
      if (this.parent) {
        ...
        this.parent.text = this.el.innerHTML;
      }
    }
  }
  update(data, index, items, context) {
    if (index == 0) console.log(context);
    this.parent = context;
    this.value = data.value;
    this.el.setAttribute('value', this.value);
    this.el.innerHTML = data.caption || data.text || data.value;
